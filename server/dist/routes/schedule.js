...
router.post('/slots', auth_1.authenticate, async (req, res) => {
    const { startTime, endTime } = req.body;
    const userId = req.user.userId;
    try {
        const slot = await prisma_1.default.availabilitySlot.create({
            data: {
                userId,
...
// Get My Slots
router.get('/slots', auth_1.authenticate, async (req, res) => {
    const userId = req.user.userId;
    const slots = await prisma_1.default.availabilitySlot.findMany({ where: { userId } });
    res.json(slots);
});
// Get Match's Slots
router.get('/slots/:matchId', auth_1.authenticate, async (req, res) => {
    ...
    const match = await prisma_1.default.match.findUnique({ where: { id: matchId } });
    if (!match)
...
router.post('/invite', auth_1.authenticate, async (req, res) => {
    ...
    const userId = req.user.userId;
    try {
        const invite = await prisma_1.default.callInvite.create({
            data: {
                matchId,
                slotId,
                proposerId: userId,
...
router.post('/accept/:inviteId', auth_1.authenticate, async (req, res) => {
    const inviteId = parseInt(req.params.inviteId);
    try {
        const invite = await prisma_1.default.callInvite.findUnique({
            where: { id: inviteId },
            include: { slot: true }
        });
        if (!invite)
        ...
        // Create Booking
        const booking = await prisma_1.default.callBooking.create({
            data: {
                inviteId,
        ...
        await prisma_1.default.callInvite.update({
            where: { id: inviteId },
